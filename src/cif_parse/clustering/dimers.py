            ...
            while pending:
                ...
                for candidate in pending[1:]:
                    ...
                    if pair_key not in alignment_cache:
                        try:
                            result = runner(
                                extracted_structures[representative.dimer_observation_id],
                                extracted_structures[candidate.dimer_observation_id],
...
def build_dimer_signature_clusters(
    *,
    ...
    observations = collect_dimer_observations(case_dirs, monomer_assignments)
    outdir = Path(outdir)
    outdir.mkdir(parents=True, exist_ok=True)
